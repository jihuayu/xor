...
mod tests {
    ...
    fn test_format_size() {
        ...
        assert_eq!(format_size(1099511627776), "1024.00 GB"); // 实际函数不支持 TB
    }

    #[test]
    fn test_compute_file_hash_simd() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let test_file = create_test_file(temp_dir.path(), "test.txt", b"Hello, World!")?;

        // 计算哈希
        let hash1 = compute_file_hash_simd(&test_file)?;
        let hash2 = compute_file_hash_simd(&test_file)?;

        // 验证哈希一致性
        assert_eq!(hash1, hash2);
        assert_eq!(hash1.len(), 64); // SHA256 哈希长度

        // 验证不同内容产生不同哈希
        let test_file2 = create_test_file(temp_dir.path(), "test2.txt", b"Different content")?;
        let hash3 = compute_file_hash_simd(&test_file2)?;
    ...
    #[test]
    fn test_get_modified_time() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let test_file = create_test_file(temp_dir.path(), "test.txt", b"test")?;

        let modified_time = get_modified_time(&test_file)?;

        // 验证时间格式
        assert!(modified_time.len() > 0);
        assert!(modified_time.contains("-"));
        assert!(modified_time.contains(":"));
    ...
    #[test]
    fn test_compress_and_encrypt_mt() -> Result<()> {
        let temp_dir = TempDir::new()?;

        // 创建测试输入文件（需要足够大才能看到压缩效果）
        let test_data = b"Hello, World! ".repeat(1000);
        let input_file = create_test_file(temp_dir.path(), "input.txt", &test_data)?;
        let output_file = temp_dir.path().join("output.bin");

        // 加密压缩
        let password = "test_password_123";
        compress_and_encrypt_mt(&input_file, &output_file, password)?;
    ...
    #[test]
    fn test_compress_and_encrypt_empty_file() -> Result<()> {
        let temp_dir = TempDir::new()?;

        // 创建空文件
        let input_file = create_test_file(temp_dir.path(), "empty.txt", b"")?;
        let output_file = temp_dir.path().join("output.bin");
    ...
    #[test]
    fn test_write_manifest_empty() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let manifest_path = temp_dir.path().join("manifest.csv");
